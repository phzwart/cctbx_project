...
def check_eigenvalue(adp: List[float], x: float) -> None:
    """
    Check if the given eigenvalue satisfies the characteristic equation.
    
    Args:
        adp (List[float]): Anisotropic displacement parameters [U11, U22, U33, U12, U13, U23]
        x (float): Eigenvalue to check
        
    Raises:
        AssertionError: If the characteristic equation is not satisfied within tolerance
    """
    r = -adp[0] - adp[1] - adp[2]
...
def check_eigenvector(adp: List[float], x: float, v: List[float]) -> None:
    """
    Check if the given eigenvector is correct for the eigenvalue.
    
    Args:
        adp (List[float]): Anisotropic displacement parameters [U11, U22, U33, U12, U13, U23]
        x (float): Eigenvalue
        v (List[float]): Eigenvector to check
        
    Raises:
        AssertionError: If the eigenvector is not correct within tolerance
    """
    v = matrix.col(v)
    assert approx_equal(
        (matrix.sym(sym_mat3=adp) * v * (1./x)).elems, v.elems, 1.e-4)

def exercise_interface() -> None:
    """
    Exercise the adptbx interface functions for ADP transformations.
    
    Tests various ADP parameter transformations including:
    - U to B factor conversions
    - U_star, U_cart, U_cif, and beta transformations
...
def u_star_minus_u_iso_airlie(unit_cell: Any, u_star: List[float]) -> List[float]:
    """
    Calculate u_star minus u_iso using Airlie's method.
    ...
    Returns:
        List[float]: U_star minus U_iso parameters
    """
...
def u_star_minus_u_iso_ralf(unit_cell: Any, u_star: List[float]) -> List[float]:
    """
    Calculate u_star minus u_iso using Ralf's method.
...
def exercise_factor_u_star_u_iso() -> None:
    """
    Exercise the factor_u_star_u_iso function with random test cases.
    
    Tests the factorization of U_star into isotropic and anisotropic components
    using both Airlie's and Ralf's methods to ensure consistency.
    """
...
def exercise_debye_waller() -> None:
    """
    Exercise Debye-Waller factor calculations.
    
    Tests various Debye-Waller factor calculations including:
    - U_iso Debye-Waller factors
    - U_cif Debye-Waller factors
    - U_star gradient and curvature calculations
    """
...
def exercise_grad_u_transformations() -> None:
    """
    Exercise gradient U transformations between different coordinate systems.
    
    Tests transformations of gradient U parameters between U_star and U_cart
    coordinate systems, including array operations.
    """
    uc = uctbx.unit_cell(
...
def exercise_eigen_core(diag: List[float]) -> None:
    """
    Core eigenvalue/eigenvector exercise with given diagonal values.
    
    Args:
        diag (List[float]): Diagonal values for the test matrix
    """
...
def exercise_eigen(n_trials: int = 100) -> None:
    """
    Exercise eigenvalue and eigenvector calculations with various test cases.
    
    Args:
        n_trials (int, optional): Number of trials for random tests. Defaults to 100.
    """
...
def exercise_random_traceless_symmetry_constrained_b_cart() -> None:
    """
    Exercise random traceless symmetry-constrained B_cart generation.
    
    Tests the generation of random traceless symmetry-constrained B_cart
    parameters and verifies that they satisfy the expected constraints.
    """
...
def exercise_misc() -> None:
    """
    Exercise miscellaneous ADP-related functions.
    
    Tests intersection calculations between scatterers and various
    ADP parameter transformations using PDB input.
    """
...
def run() -> None:
    """
    Run all test exercises.
    
    Executes all the test functions in sequence to verify the functionality
    of the adptbx module.
    """
    exercise_misc()
    exercise_interface()
